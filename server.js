...
/*
    .put(authJwtController.isAuthenticated, function (req, res) {
        ...
        Movie.findOne({Title: req.headers.title}).exec(function(err, movie) {
            if (movie !== null) {
                Movie.findOne({Title: req.body.Title}).exec(function(err, movie1) {
                    if (movie1 !== null)
                    {
                        res.json({ success: false, message: 'Failed to update a movie. '});
                    }
                    else
                    {
                        Movie.update({Title: req.headers.title}, req.body).exec(function (err) {
                            res.json({ message: 'Movie updated!' });
                        });
                    }
                ...
                movie.save(function(err) {
                    if (err) {
                        // duplicate entry
                        if (err.code == 11000)
                            return res.json({ success: false, message: 'Failed to update a movie. '});
                        else
                            return res.send(err);
                    }
                    res.json({ message: 'Movie updated!' });
...
/*
            ...
            {
                res.json({ success: false, message: 'Movie is not found. '});
            }
        });
    });
